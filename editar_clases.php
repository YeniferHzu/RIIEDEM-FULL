...
<div class="container">
<input type="hidden" name="id_clases" value="<?php $id_clases = $_GET['id_clases'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/clases_editar.php">
		
	<?php 	
	$id_clases = $_GET['id_clases'];
	$query = mysqli_query($conn, "SELECT * FROM `clases` WHERE `id_clases` = '$id_clases'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_clases);
		$id_clases=$fetch['id_clases'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_clases" value="<?php echo $fetch['id_clases']?>"/>
			<br>
			<a href="clases.php" class="link-success">Atrás</a>
		</div>
	</form> 
