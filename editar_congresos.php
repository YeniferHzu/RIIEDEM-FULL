...
<div class="container">
<input type="hidden" name="id_congresos" value="<?php $id_congresos = $_GET['id_congresos'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/congresos_editar.php">
		
	<?php 	
	$id_congresos = $_GET['id_congresos'];
	$query = mysqli_query($conn, "SELECT * FROM `congresos` WHERE `id_congresos` = '$id_congresos'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_congresos);
		$id_congresos=$fetch['id_congresos'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_congresos" value="<?php echo $fetch['id_congresos']?>"/>
			<br>
			<a href="congresos.php" class="link-success">Atrás</a>
		</div>
	</form> 
