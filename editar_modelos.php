...
<div class="container">
<input type="hidden" name="id_modelos" value="<?php $id_modelos = $_GET['id_modelos'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/modelos_editar.php">
		
	<?php 	
	$id_modelos = $_GET['id_modelos'];
	$query = mysqli_query($conn, "SELECT * FROM `modelos` WHERE `id_modelos` = '$id_modelos'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_modelos);
		$id_modelos=$fetch['id_modelos'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_modelos" value="<?php echo $fetch['id_modelos']?>"/>
			<br>
			<a href="modelos.php" class="link-success">Atrás</a>
		</div>
	</form> 
