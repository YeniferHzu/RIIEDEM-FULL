...
<div class="container">
<input type="hidden" name="id_libros" value="<?php $id_libros = $_GET['id_libros'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/libros_editar.php">
		
	<?php 	
	$id_libros = $_GET['id_libros'];
	$query = mysqli_query($conn, "SELECT * FROM `libros` WHERE `id_libros` = '$id_libros'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_libros);
		$id_libros=$fetch['id_libros'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_libros" value="<?php echo $fetch['id_libros']?>"/>
			<br>
			<a href="libros.php" class="link-success">Atrás</a>
		</div>
	</form> 
