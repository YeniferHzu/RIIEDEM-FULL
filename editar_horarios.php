...
<div class="container">
<input type="hidden" name="id_horarios" value="<?php $id_horarios = $_GET['id_horarios'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/horarios_editar.php">
		
	<?php 	
	$id_horarios = $_GET['id_horarios'];
	$query = mysqli_query($conn, "SELECT * FROM `horarios` WHERE `id_horarios` = '$id_horarios'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_horarios);
		$id_horarios=$fetch['id_horarios'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_horarios" value="<?php echo $fetch['id_horarios']?>"/>
			<br>
			<a href="horarios.php" class="link-success">Atrás</a>
		</div>
	</form> 
