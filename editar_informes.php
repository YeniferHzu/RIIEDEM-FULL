...
<div class="container">
<input type="hidden" name="id_informes" value="<?php $id_informes = $_GET['id_informes'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/informes_editar.php">
		
	<?php 	
	$id_informes = $_GET['id_informes'];
	$query = mysqli_query($conn, "SELECT * FROM `informes` WHERE `id_informes` = '$id_informes'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_informes);
		$id_informes=$fetch['id_informes'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_informes" value="<?php echo $fetch['id_informes']?>"/>
			<br>
			<a href="informes.php" class="link-success">Atrás</a>
		</div>
	</form> 
