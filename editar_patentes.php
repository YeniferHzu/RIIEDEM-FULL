...
<div class="container">
<input type="hidden" name="id_patentes" value="<?php $id_patentes = $_GET['id_patentes'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/patentes_editar.php">
		
	<?php 	
	$id_patentes = $_GET['id_patentes'];
	$query = mysqli_query($conn, "SELECT * FROM `patentes` WHERE `id_patentes` = '$id_patentes'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_patentes);
		$id_patentes=$fetch['id_patentes'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_patentes" value="<?php echo $fetch['id_patentes']?>"/>
			<br>
			<a href="patentes.php" class="link-success">Atrás</a>
		</div>
	</form> 
