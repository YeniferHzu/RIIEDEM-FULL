...
<div class="container">
<input type="hidden" name="id_otros" value="<?php $id_otros = $_GET['id_otros'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/otros_editar.php">
		
	<?php 	
	$id_otros = $_GET['id_otros'];
	$query = mysqli_query($conn, "SELECT * FROM `otros` WHERE `id_otros` = '$id_otros'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_otros);
		$id_otros=$fetch['id_otros'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_otros" value="<?php echo $fetch['id_otros']?>"/>
			<br>
			<a href="otros.php" class="link-success">Atrás</a>
		</div>
	</form> 
