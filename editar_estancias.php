...
<div class="container">
<input type="hidden" name="id_estancias" value="<?php $id_estancias = $_GET['id_estancias'];?>"/>
	<form class="m-auto w-50 mt-2 mb-2 " method="POST" enctype="multipart/form-data" action="acciones/estancias_editar.php">
		
	<?php 	
	$id_estancias = $_GET['id_estancias'];
	$query = mysqli_query($conn, "SELECT * FROM `estancias` WHERE `id_estancias` = '$id_estancias'") or die(mysqli_error());
	while($fetch = mysqli_fetch_array($query)) {
		?>

		<?php 
		$conexion=conexion();
		$fetch=datos($conexion,$id_estancias);
		$id_estancias=$fetch['id_estancias'];
		$nombre=$fetch['nombre'];
		...
		<div align="center">
			...
			<input type="hidden" name="id_estancias" value="<?php echo $fetch['id_estancias']?>"/>
			<br>
			<a href="estancias.php" class="link-success">Atrás</a>
		</div>
	</form> 
